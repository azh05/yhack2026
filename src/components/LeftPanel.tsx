...
import { getSeverityColor, type ConflictZone } from "@/data/conflicts";
  ...
  const sortedZones = useMemo(() => {
    ...
    return zones
      .filter((zone) => {
        ...
        if (q) {
          const haystack =
            `${zone.name} ${zone.country} ${zone.region} ${zone.primaryType}`.toLowerCase();
        ...
        <button
          ...
          title={
            !isLoggedIn
              ...
              : `${watchlist.length} watched countries`
          }
                ...
                >
                  ...
                  <div
                    className={`absolute left-0 top-1 bottom-1 rounded-r transition-all duration-300 ${
                      isSelected
                        ? "w-[3px] opacity-100"
                        : "w-[2px] opacity-0 group-hover:opacity-70"
                    }`}
                    style={{
