...
export default function TimelineBar({ onDateChange, eventCount, earliestDate }: TimelineBarProps) {
  ...
  const TOTAL_DAYS = Math.max(1, Math.floor((END_DATE.getTime() - START_DATE.getTime()) / (1000 * 60 * 60 * 24)));
  const NOW_PCT = 100;
  ...
  const currentMode = TIMELAPSE_MODES.find(m => m.id === timelapseMode) || TIMELAPSE_MODES[2];
  ...
  const stepBack = () => setProgress(prev => Math.max(0, prev - daysToPct(currentMode.days)));
  const stepForward = () => setProgress(prev => Math.min(100, prev + daysToPct(currentMode.days)));
    ...
    <div className="fixed bottom-0 left-0 right-0 z-40 glass border-t border-white/[0.04]">
      ...
      <div className="px-4 pt-2">
        ...
        >
          ...
          {/* Month labels */}
          {monthLabels.map((m, i) => (
            <div key={`label-${i}`} className="absolute top-0" style={{ left: `${m.pct}%` }}>
